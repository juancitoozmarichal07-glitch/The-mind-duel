...
const config = {
    questionsLimit: 20,
    typewriterSpeed: 20,
    backendURL: backendURL: 'https://the-oracle-game.onrender.com/api/oracle',
...
async function getSuggestions() {
    ...
    // Si ya se agotaron los usos totales de sugerencias (5)
    if (state.suggestionsLocked || state.suggestionsUsed >= state.maxSuggestions) {
    ...
    try {
        ...
        if (data.suggestions && data.suggestions.length > 0) {
            // Verificar si son nuevas sugerencias
            const areNew = JSON.stringify(data.suggestions) !== JSON.stringify(state.lastSuggestions);

            if (areNew) {
                // Nuevo ciclo: reiniciar contador y guardar sugerencias
                state.remainingRefreshes = 3;
                state.lastSuggestions = data.suggestions;
                state.currentSuggestions = data.suggestions;
            } else {
                // Mismas sugerencias: consumir un refresco si quedan
                if (state.remainingRefreshes > 0) {
                    state.remainingRefreshes--;
                }
            }

            // Mostrar pop-up con las sugerencias actuales y los refrescos restantes
            showSuggestionsPopup(state.currentSuggestions, state.remainingRefreshes);
...
function showSuggestionsPopup(suggestions, refreshesLeft) {
    ...
    suggestions.forEach(suggestion => {
        ...
        item.onclick = () => {
            ...
            // Reiniciar ciclo porque se usó una sugerencia
            state.remainingRefreshes = 3;
            state.lastSuggestions = [];
            updateButtonStates();
            el.questionInput.focus();
...
function addMessageWithBubble(text, sender, callback, skipTypewriter = false) {
    ...
    if (sender === 'brain') {
        ...
        senderName.textContent = ' Sistema:';
    }
...
console.log('🧠 MIND DUEL - Versión Arcade con temporizador y sonidos');
...
if (el.exitBtn) {
    ...
    el.exitBtn.addEventListener('click', () => {
        ...
        farewell.innerHTML = `
            ...
            <p>Si deseas volver a buscar conocimiento,<br>deberás invocar al Cerebro nuevamente<br>recargando la página.</p>
